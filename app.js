/* ==========================================================================
   THE PODIUM  —  app.js  (the topic machine)
   Pull the lever, the reel shuffles and lands on a prompt, then practise:
   mode, prep time, timer, speech capture, target words, word bank, history.
   No server, no keys.
   ========================================================================== */
...
(function () {
  ...
  const el = {
    ...
    slot: document.querySelector(".slot"), reelTrack: $("reelTrack"),
    lever: $("lever"), drawBtn: $("drawBtn"),
    ...
    research: $("research"), researchQuery: $("researchQuery"), scoutBtn: $("scoutBtn"),
    researchResults: $("researchResults"), wikiCard: $("wikiCard"), researchLinks: $("researchLinks"),
  ...
  const ITEM_H = 58;                 // matches .reel-item height
  ...
  const windowCenter = () => el.reelTrack.parentElement.clientHeight / 2 - ITEM_H / 2;
  function idleReel() {
    el.reelTrack.style.transition = "none";
    el.reelTrack.innerHTML = "";
    const d = document.createElement("div"); d.className = "reel-item pick"; d.textContent = "Pull the lever!";
    el.reelTrack.appendChild(d);
    el.reelTrack.style.transform = "translateY(" + windowCenter() + "px)";
  }
  function pull() {
    if (spinning) return;
    const p = pool();
    ...
    setTimeout(() => el.lever.classList.remove("pulled"), 480);
    el.slot.classList.add("spinning"); el.slot.classList.remove("landed");
    ...
    const start = windowCenter();
    el.reelTrack.style.transition = "none";
    el.reelTrack.style.transform = "translateY(" + start + "px)";
    void el.reelTrack.offsetHeight;
    const target = start - landing * ITEM_H;
    el.reelTrack.style.transition = "transform 2.6s cubic-bezier(0.15,0.85,0.25,1)";
    ...
    const done = () => {
      ...
      el.slot.classList.remove("spinning"); el.slot.classList.add("landed");
      spinning = false; el.drawBtn.disabled = false;
      chime(); currentTopic = chosen; render(chosen);
    };
  ...
  function render(t) {
    ...
    el.marquee.classList.remove("show"); void el.marquee.offsetWidth; el.marquee.classList.add("show");
  ...
  // ==== RESEARCH (real papers, no key, no backend) =======================
  const SCHOLAR = (q) => "https://scholar.google.com/scholar?q=" + encodeURIComponent(q);
  const RGATE   = (q) => "https://www.researchgate.net/search?q=" + encodeURIComponent(q);
  const SEMSCH  = (q) => "https://www.semanticscholar.org/search?q=" + encodeURIComponent(q) + "&sort=relevance";

  function cleanQuery(t) {
    if (t.type === "concept") return t.task;
    return t.task
      .replace(/\b(argue (your side|it|that)|discuss|take a (side|position)|make the case( for)?|explain|defend|challenge it|speak)\b/gi, " ")
      .replace(/[\u2014\u2013-]\s*[^.?]*[.?]?\s*$/, " ")
      .replace(/[?.]/g, " ").replace(/\s+/g, " ").trim();
  }
  function buildResearchLinks(q) {
    el.researchLinks.innerHTML = "";
    [["Google Scholar", SCHOLAR(q)], ["ResearchGate", RGATE(q)], ["Semantic Scholar", SEMSCH(q)]].forEach(([label, href]) => {
      const a = document.createElement("a"); a.href = href; a.target = "_blank"; a.rel = "noopener"; a.textContent = label;
      el.researchLinks.appendChild(a);
    });
  }
  function setupResearch(t) {
    const q = cleanQuery(t);
    el.researchQuery.value = q;
    buildResearchLinks(q);
    el.researchResults.innerHTML = ""; el.wikiCard.hidden = true;
    if (mode === "research") { el.research.hidden = false; scout(); }
    else { el.research.hidden = true; }
  }
  function toggleResearch() {
    if (!currentTopic) { el.research.hidden = true; return; }
    if (mode === "research") { el.research.hidden = false; if (!el.researchResults.children.length) scout(); }
    else { el.research.hidden = true; }
  }
  async function scout() {
    const q = el.researchQuery.value.trim();
    if (!q) return;
    buildResearchLinks(q);
    el.researchResults.innerHTML = "<p class=\"research-status\">Searching papers\u2026</p>";
    el.wikiCard.hidden = true;
    try {
      const url = "https://api.crossref.org/works?rows=6&sort=relevance&select=title,author,container-title,issued,DOI,URL,is-referenced-by-count&query=" + encodeURIComponent(q);
      const r = await fetch(url);
      if (!r.ok) throw new Error("http " + r.status);
      const data = await r.json();
      const items = (data.message && data.message.items) || [];
      if (!items.length) el.researchResults.innerHTML = "<p class=\"research-status\">No papers found for that phrase. Try the links below, or edit the search.</p>";
      else { el.researchResults.innerHTML = ""; items.forEach((it) => el.researchResults.appendChild(paperCard(it))); }
    } catch (e) {
      el.researchResults.innerHTML = "<p class=\"research-status\">Couldn't load papers in-app (offline, or the source blocked it). The search links below still work.</p>";
    }
    if (currentTopic && currentTopic.type === "concept") loadWiki(currentTopic.task);
  }
  function paperCard(it) {
    const a = document.createElement("a");
    a.className = "paper"; a.target = "_blank"; a.rel = "noopener";
    a.href = it.URL || (it.DOI ? "https://doi.org/" + it.DOI : "#");
    const title = (it.title && it.title[0]) || "Untitled";
    const authors = (it.author || []).slice(0, 3).map((au) => [au.given, au.family].filter(Boolean).join(" ")).join(", ");
    ...
    const cites = it["is-referenced-by-count"];
    const meta = [authors, journal, year].filter(Boolean).join(" \u00B7 ") + (cites ? "  \u2014  " + cites + " citations" : "");
    const tt = document.createElement("div"); tt.className = "paper-title"; tt.textContent = title;
    const mm = document.createElement("div"); mm.className = "paper-meta"; mm.textContent = meta;
    a.appendChild(tt); a.appendChild(mm);
    return a;
  }
  async function loadWiki(term) {
    try {
      ...
      if (!d.extract || d.type === "disambiguation") return;
      el.wikiCard.innerHTML = "";
      const h = document.createElement("h4"); h.textContent = "In brief \u00B7 " + (d.title || term);
      const p = document.createElement("p"); p.textContent = d.extract;
      const a = document.createElement("a"); a.target = "_blank"; a.rel = "noopener";
      a.href = (d.content_urls && d.content_urls.desktop && d.content_urls.desktop.page) || ("https://en.wikipedia.org/wiki/" + encodeURIComponent(term));
      a.textContent = "Read on Wikipedia \u2197";
      el.wikiCard.appendChild(h); el.wikiCard.appendChild(p); el.wikiCard.appendChild(a);
  ...
  function init() {
    ...
    requestAnimationFrame(idleReel);
    ...
    el.drawBtn.addEventListener("click", pull);
    el.lever.addEventListener("click", pull);
    el.startBtn.addEventListener("click", startTimer);
    el.resetBtn.addEventListener("click", () => setDuration(targetSeconds / 60));
    el.recordBtn.addEventListener("click", toggleRecording);
    el.scoutBtn.addEventListener("click", scout);
    el.researchQuery.addEventListener("keydown", (e) => { if (e.key === "Enter") scout(); });
    ...
    document.addEventListener("keydown", (e) => { if (e.code === "Space" && e.target === document.body) { e.preventDefault(); pull(); } });
    window.addEventListener("resize", () => { if (!spinning && el.stage.hidden) idleReel(); });
  }
